...
2. 自定义窗体提示框(参考Toast源码)
	...
	params.format = PixelFormat.TRANSLUCENT;	//源码中这里是TYPE_TAOST但是这里为了下面要进行点击拖动事件，而Toast不能拖动，所以这里改成了TYPE_PRIORITY_PHONE,这是一个系统类型的提示框，使用这个提示框必须要申请权限,android.permission.SYSTEM_ALERT_WINDOW
...
6. 触摸和双击同时发生时候的返回值
	...
	iv_drag_view.setOnTouchListener(new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			//这里对于触摸事件应该是返回true为什么这里返回false呢，因为这里这一个控件同时实现了点击和触摸这两个事件，如果返回true，那么就不可能发生点击事件了，所以对于同时实现点击和触摸的控件返回值要为false
			return false;
		}
	});
	```
