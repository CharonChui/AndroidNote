...
例如，用普通的方式，连续请求三个接口，需要三层嵌套。 
但是使用协程就会看起来像同步代码一样的实现，但实际是异步的:    

```kotlin
suspend fun loadUserData(suerId: String) {
    val user = apiService.getUser(userId)        // 挂起，不堵塞线程
    val posts = apiService.getUserPosts(user.id) // 挂起，不堵塞线程
    val comments = apiService.getPostComents(posts[0].id) // 挂起，不堵塞线程
    updateUI(user, posts, comments)  // 回到主线程更新UI
}
```
