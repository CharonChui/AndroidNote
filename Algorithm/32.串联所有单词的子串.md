...
class Solution {
    fun findSubstring(s: String, words: Array<String>): List<Int> {
        ...
        // 窗口总长度
        val totalLen = wordLen * wordsCount
        // s 比答案还短，不可能有解
        if (s.length < totalLen) {
            return result
        }

        // 用HashMap记录匹配words中的个数和出现的次数
        val need = HashMap<String, Int>()
        for (w in words) {
            need[w] = need.getOrDefault(w, 0) + 1
        }

        // ---- 多个滑动窗口起点: 只需开 wordLen 条「车道」（偏移量）----
        for (offset in 0 until wordLen) {
            ...
            // right 每次读入一整张卡片，需保证能切出完整单词
            while (right + wordLen <= s.length) {
                // 取出 right 处的一张卡片，right 跳过它
                val word = s.substring(right, right + wordLen)
                right += wordLen

                if (!need.containsKey(word)) {
                    // 情况 A：假卡片（不在账本里）→ 整个窗口作废，从 right 重开
                    window.clear()
                    count = 0
                    left = right
                } else {
                    ...
                    // 虽然有这个卡但是这张卡片超量了 → 从队头不断踢，直到它降回合法数量
...
- [下一题](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/33.%E6%9C%80%E5%B0%8F%E8%A6%86%E7%9B%96%E5%AD%90%E4%B8%B2.md)
