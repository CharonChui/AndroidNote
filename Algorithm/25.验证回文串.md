...
class Solution {
    fun isPalindrome(s: String): Boolean {
        ...
        while (leftIndex < rightIndex) {
            if (!Character.isLetterOrDigit(s[leftIndex])) {
                leftIndex ++
                continue
            }

            if (!Character.isLetterOrDigit(s[rightIndex])) {
                rightIndex --
                continue
            }

            if (!s[leftIndex].equals(s[rightIndex], true)) {
                return false
            } else {
                leftIndex++
                rightIndex--
...
- [下一题](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/26.%E5%88%A4%E6%96%AD%E5%AD%90%E5%BA%8F%E5%88%97.md)
