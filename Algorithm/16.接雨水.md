...
class Solution {
    fun trap(height: IntArray): Int {
        ...
        var rightHeight = IntArray(length) { 0 }
        var lefthHeight = IntArray(length) { 0 }

        var currentLeftHeight = 0
        var currentRightHeight = 0
        // [0,1,0,2,1,0,1,3,2,1,2,1]
        for (i in 1..<length) {
            currentLeftHeight = max(currentLeftHeight, height[i - 1])
            lefthHeight[i] = currentLeftHeight
        }
        for (i in length - 2 downTo 0) {
            currentRightHeight = max(currentRightHeight, height[i + 1])
            rightHeight[i] = currentRightHeight
        }

        var count = 0

        for (i in 0 ..< length) {
            var current = min(rightHeight[i], lefthHeight[i]) - height[i]
            if (current > 0) {
                count += current
            }
        }
...
- 维护两个指针left和right，以及两个变量leftMax(左侧已扫描的最大高度)和rightMax(右侧已扫描的最大高度)，初始时left=0,right=length−1,leftMax是0坐标的值,rightMax是length - 1坐标的值。
- 所以这个时候，leftMax是左侧最大高度的最小值，rightMax是右侧最大高度的最小值
- 指针left向右移动，指针right向左移动，在移动指针的过程中维护两个变量leftMax和rightMax的值。    

当两个指针没有相遇时，进行如下操作：

- 使用height[left]和height[right]的值去判断是不是超过当前的值来更新leftMax和rightMax的值

- 如果`leftMax<rightMax`，左边的高度是短板，能够接的数量由左边高度决定，下标`left`处能接的雨水量等于`leftMax−height[left]`，将下标`left`处能接的雨水量加到能接的雨水总量，然后将`left`加1（即向右移动一位）

- 如果`leftMax≥rightMax`，那右边的高度是短板，能够接的水量由右边的高度决定，下标`right`处能接的雨水量等于`rightMax−height[right]`，将下标right处能接的雨水量加到能接的雨水总量，然后将right减1（即向左移动一位）

- 当两个指针相遇时，就都循环完了，即可得到能接的雨水总量。
...
class Solution {
    fun trap(height: IntArray): Int {
        ...
        while (leftIndex < rightIndex) {
            if (leftMax <= rightMax) {
                count += max(leftMax - height[leftIndex], 0)
                leftIndex ++
                leftMax = max(leftMax, height[leftIndex])
            } else {
                count += max(rightMax - height[rightIndex], 0)
                rightIndex --
...
- [下一篇](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/17.%E7%BD%97%E9%A9%AC%E6%95%B0%E5%AD%97%E8%BD%AC%E6%95%B4%E6%95%B0.md)
