...
class Solution {
    fun rotate(matrix: Array<IntArray>) {
        ...
        // ① 转置：沿主对角线交换 (i,j) 和 (j,i)
        // 注意 j 从 i 开始，只处理上三角，避免交换两次又换回来
        for (i in 0 until n) {
            for (j in i until n) {
                val temp = matrix[i][j]
                matrix[i][j] = matrix[j][i]
...
- [下一题](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/37.%E7%9F%A9%E9%98%B5%E7%BD%AE%E9%9B%B6.md)
