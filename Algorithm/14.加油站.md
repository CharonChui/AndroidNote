...
class Solution {
    fun canCompleteCircuit(gas: IntArray, cost: IntArray): Int {
        ...
        for (i in gas.indices) {
            ...
            if (current < 0) {
                index = i + 1
                current = 0 // 注意，重新开始算起点了，所以必须要把current重置为0
            }
        }
        if (total < 0) {
            return -1
...
核心逻辑：如果从 idx 出发，走到第 i 站时油量为负，说明 idx 到 i 之间的任何一站都不能作为起点。

原因：从 idx 出发时油量是 0，途中每一站到达时油量都是非负的（否则上一次就已经重置了）。如果从中间某站 k（idx ≤ k ≤ i）出发，到达第 i 站的油量 = 从 idx 出发到第 i 站的油量 - 从 idx 出发到第 k 站的油量，后半部分 ≥ 0，所以结果只会更差。

因此直接跳过，从 i + 1 重新尝试。

##### 为什么要 % gas.length

只是防止最后一站（i = gas.length - 1）时 i + 1 越界。

实际上 totalNum < 0 已经处理了无解的情况，有解时 idx 不会真的等于 gas.length，所以 % gas.length 只是防御性写法，去掉也不影响正确性。



- [下一篇](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/15.%E5%88%86%E5%8F%91%E7%B3%96%E6%9E%9C.md)
