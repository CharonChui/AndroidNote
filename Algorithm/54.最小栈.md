...
要做到O(1)，关键问题是: pop之后，如何知道剩余元素的最小值是多少？   

单独维护一个minValue变量肯定是不够的，因为pop掉的恰好是当前最小值时，无法知道下一个最小值是什么。     



所有解决方案就是: 借用一个辅助栈min_stack，用于存获取stack中每个时刻的最小值。    
...
class MinStack() {
    ...
    fun push(value: Int) {
        ...
        if (minStack.isEmpty()) {
            ...
            minStack.add(minOf(minStack.last(), value))
        }
        stack.add(value)
    }
...
- [下一题](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/55.逆波兰表达式求值.md)
