...
class Solution {
    fun wordPattern(pattern: String, s: String): Boolean {
        val split = s.split(" ")
        if (pattern.length != split.size) {
            return false
        }
        val p2sHash = HashMap<Char, String>()
        val s2pHash = HashMap<String, Char>()

        for (i in pattern.indices) {
            if (p2sHash.containsKey(pattern[i])) {
                if (p2sHash[pattern[i]] != split[i]) {
                    return false
                }
            } else {
                p2sHash[pattern[i]] = split[i]
            }

            if (s2pHash.containsKey(split[i])) {
                if (s2pHash[split[i]] != pattern[i]) {
                    return false
                }
            } else {
                s2pHash[split[i]] = pattern[i]
            }
        }
...
- [下一题](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/42.有效的字母异位词.md)
