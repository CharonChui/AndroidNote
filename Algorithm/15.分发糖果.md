...
class Solution {
    fun candy(ratings: IntArray): Int {
        ...
        val candyArray = IntArray(length) { 1 }
        // 左规则： 从左向右，右边比左边高则加1
        ...
        count += candyArray[length - 1]
        // 右规则： 从右向左，左边比右边高则去最大值
        for (i in length - 2 downTo 0) {
            if (ratings[i] > ratings[i + 1]) {
                candyArray[i] = maxOf(candyArray[i], candyArray[i + 1] + 1)
            }
            count += candyArray[i]
        }
...
```
规律：下降长度为 k 时，需要补的量是 (k-1) + (k-2) + ... + 1 + 0 = k*(k-1)/2。

所以正确公式是 k*(k-1)/2，不是 k*(k+1)/2。

加上默认的每人1颗（共 k 颗），下降段总糖果 = k + k*(k-1)/2 = k*(k+1)/2。
```
    ...
    fun candy(ratings: IntArray): Int {
        ...
        for (i in 1 until length) {
            if (ratings[i] > ratings[i - 1]) {
                ...
                down ++
                up = 0
                // peak >= down: 山峰够高，加 down（不补偿）
                // peak < down: 山峰不够高，加 down+1（补偿山峰）
                if (peak >= down) {
                    total += down
                } else {
                    total += down + 1
...
- [下一篇](https://github.com/CharonChui/AndroidNote/blob/master/Algorithm/16.%E6%8E%A5%E9%9B%A8%E6%B0%B4.md)
